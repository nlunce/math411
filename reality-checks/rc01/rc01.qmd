...
for num_roots, p2_list in root_counts.items():
    if p2_list:
        ...
        # Adjust the splitting condition to group consecutive p2 values
...
# After all intervals are collected, print only the first and last interval for each number of poses
for num_roots, intervals in intervals_dict.items():
    if intervals:
        ...
        if len(intervals) == 1:
            ...
            # Check if the interval ends at p2_max to represent infinity
            if np.isclose(p2_end, p2_max, atol=tol):
                p2_end_str = "infinity"
            else:
                p2_end_str = f"{p2_end:.2f}"
            print(f"  p2 from {p2_start:.2f} to {p2_end_str}")
        else:
            # Multiple intervals exist; print only the first and last intervals
            first_interval = intervals[0]
            last_interval = intervals[-1]
            
            # First interval
            p2_start, p2_end = first_interval
            if np.isclose(p2_end, p2_max, atol=tol):
                p2_end_str = "infinity"
            else:
                p2_end_str = f"{p2_end:.2f}"
        ...
        print(f"\nNo intervals found with {num_roots} poses.")
```
