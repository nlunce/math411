...
2. **Objective Function**: Rewrite this condition as $f(t) = 0$, where
   ...
   $$
   f(t) = \frac{\text{Arc length from } t = 0 \text{ to } t}{\text{Total arc length}} - s
   $$
...
plt.scatter(x_points, y_points, color="blue", s=30, zorder=3)

# Add labels for the points, slightly offset to avoid overlap with the line
for (px, py, label) in zip(x_points, y_points, labels):
    plt.text(px + 0.1, py + 0.1, label, fontsize=8, ha='center')
...
plt.xticks([-1, 0, 1])
plt.yticks([0, 1, 2])
plt.title("Figure 5.6: Parametrized Curve with Labeled Points")
plt.grid()
plt.show()
```
...
# Plot function for equipartitions
def plot_styled_curve(n):
    ...
    # Define the key points using equipartition
    ...
    plt.xticks([-1, 0, 1])
    plt.yticks([0, 1, 2])
    plt.title(f"Equipartitioned Path for n = {n}")
    plt.grid()
    plt.show()

# Call the function to plot with n=4 and n=20
plot_styled_curve(4)
...
def compare_performance(target_length):
    ...
    bisection_find_t(target_length)
    bisection_time = time.time() - start_time_bisect

    start_time_newton = time.time()
    newton_find_t(target_length, initial_guess=0.5)
    newton_time = time.time() - start_time_newton

    print(f"Bisection Method Time: {bisection_time:.6f} seconds")
    print(f"Newton's Method Time: {newton_time:.6f} seconds")
...
# Plot settings for original speed
ax1.plot(x_vals_original_speed, y_vals_original_speed, color="skyblue")
original_point, = ax1.plot([], [], 'bo')  # Blue points for original speed
...
# Plot settings for constant speed
ax2.plot(x_vals_constant_speed, y_vals_constant_speed, color="skyblue")
constant_point, = ax2.plot([], [], 'bo')  # Blue points for constant speed
...
# Define update functions for each animation
...
# Get the number of frames based on data length
num_frames = len(x_vals_original_speed)

# Create a single animation showing both plots side by side
...
The chosen path is defined by the following parametric equations:
