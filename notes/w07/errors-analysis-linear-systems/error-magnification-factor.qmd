...
# Define values for true solution, approximate solution, and residual
x_true = np.array([2, 1])
x_a = np.array([1, -1])
b = np.array([3, 2])
A = np.array([[1, 1], [3, -4]])
...
# Combine all vectors to determine common plot limits
all_vectors = [
...
# Create figure with white background and reduced size
plt.style.use('seaborn-v0_8-whitegrid')
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 8))  # Keep side-by-side comparison
fig.patch.set_facecolor('white')

# Color scheme with improved visibility
colors = {
    'b': '#1f77b4',        # blue
    'Ax_a': '#2ca02c',     # green
    'residual': '#d62728', # red
...
# Left subplot: Input space (showing b, Ax_a, and residual)
input_vectors = [
...
for start, end, color, name in input_vectors:
    ...
    # Calculate vector direction and perpendicular direction
    ...
    # Default offset for labels
    perp_direction = np.array([-direction[1], direction[0]])
    offset = perp_direction * 0.5  # Default offset size

    # Specific adjustment for $A\mathbf{x}_a$
    if name == r'$A\mathbf{x}_a$':
        offset += np.array([-0.3, 0])  # Push label to the left
...
# Right subplot: Solution space (showing x, x_a, and forward error)
solution_vectors = [
...
for start, end, color, name in solution_vectors:
    ...
    vector = np.array(end) - np.array(start)
    mid_point = np.array(start) + vector / 2
    offset = np.array([0.3, 0.3])  # Default offset

    # Specific adjustment for $x_a$
    if name == r'$\mathbf{x}_a$':
        offset += np.array([-0.1, -0.3])  # Push label downward

    # Place label
    ax2.text(mid_point[0] + offset[0], mid_point[1] + offset[1],
             name, color=color, fontsize=10, fontweight='bold', zorder=6)

# Configure both subplots with the same limits
for ax, title in zip([ax1, ax2], ['Input Space (Backward Error)', 'Solution Space (Forward Error)']):
    ax.set_aspect('equal')
    # Add padding to ensure vectors are visible
...
plt.tight_layout()
plt.subplots_adjust(top=0.87, bottom=0.15, right=0.95, wspace=-0.4)  # Adjusted to make room for legend and title
