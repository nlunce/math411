...
The **infinity norm** (also called the maximum norm or $\ell_\infty$-norm) measures the size of a vector by taking the maximum absolute value of its components. Unlike the Euclidean or Taxicab norms, which involve summing components, the infinity norm focuses on the "largest step" in any single direction.
