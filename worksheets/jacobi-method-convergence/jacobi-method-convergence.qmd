...
Since $\lambda$ is an arbitrary eigenvalue, then $|\lambda|_{\text{max}} < 1$. In other words, the spectral radius $\rho(D^{-1}R) < 1$. Thus, by the **Spectral Radius Convergence Theorem** (Theorem A.7), the Jacobi Method (iteration with $A = D - D^{-1}R$) converges for any starting point $\mathbf{x}_0$.
